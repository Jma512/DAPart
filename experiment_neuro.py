...
def get_upload_speed():
    ...
    # 创建一个虚拟文件
    file_content = b"Hello, world!" * 1024 * 20  # 生成一个大小为1MB的文件
    file_size = len(file_content) / (1024 * 1024)  # 计算文件大小
    ...
    try:
        ...
        # 发送POST请求上传文件
        response = requests.post(url, data=virtual_file, timeout=2)

        # 记录结束时间
        end_time = time.time()

        upload_time = end_time - start_time  # 计算上传所需的时间
        upload_speed = file_size / upload_time * 10  # 计算上传速度
        print(upload_time)
        print(f"上传速度: {upload_speed:.2f} Mbps")
        return upload_speed
    except requests.exceptions.Timeout:
        ex_time = time.time() - start_time  # 计算上传所需的时间
        ex_speed = 0
...
def inference(submodel1, img, util, size=224):
    ...
    with torch.no_grad():
        ...
        try:
            response = requests.post('http://10.12.55.70:8022/startEdge', data=serialized_data, timeout=10)
...
def edge_computing(model, label_path, data_path):
    ...
    if model == "VGG16":
        util2, result, total_time = VGG16.edge_computing_vgg16(label_path, data_path, util2)
    elif model == "MobileNetV2":
        util2, result, total_time = MobileNetV2.edge_computing_mobilenetv2(label_path, data_path, util2)
    elif model == "ResNet50":
        util2, result, total_time = ResNet50.edge_computing_resnet50(label_path, data_path, util2)
    # elif model == "AlexNet":
    #     util2, result, total_time = AlexNet.edge_computing_alexnet(label_path, data_path, util2)
    # elif model == "SwinTransformer":
    #     path = ""
    # elif model == "MobileViT":
    #     util2, result, total_time = MobileViT.edge_computing_mobilevit(label_path, data_path, util2)
    else:
        print("No model type!")
        sys.exit(0)
...
def split_computing(submodel1, label_path, data_path, util, size=224):
    label_dict = json.load(open(label_path, "r"))
    inverted_dict = {value[0]: key for key, value in label_dict.items()}
    ...
    return accuracy, total_time, util
...
def main(modeltype, label_path, data_totalpath, buffer, epoches):
    ...
    for i in range(epoches):
        ...
        for childpath in os.listdir(data_totalpath):
            ...
            percent, strength = get_signal_level_wireless()  # 信道强度百分比，信道质量（dbm）
            ...
            dict = {'VGG16': 40, 'AlexNet': 8, 'MobileNetV2': 26, 'ResNet50': 18, 'MobileViT': 13}
            ...
            if position == 0:
                ...
                accuracy, total_time, util = split_computing(submodel1, label_path, data_path, util, size)
            ...
            next_state, action, reward, average = agent.env_step_reward(state, action, total_time, energy, [modeltype, transmission_rate])
            print(state, "  ", action, "  ", reward)
            print("总时间", total_time, "  ", "能耗", energy)
    ...
    fileName = 'local_time_data_neuro.xlsx'
    op_toExcel(util.gettimelist(), fileName)
    print("本地结果已保存")
    try:
        response = requests.post(f'http://{server_ip}:{server_port}/writeEXCEL_neuro', data=[], timeout=3)
        print(response.text)
    except requests.exceptions.Timeout:
        print("writeEXCEL timeout")
    except requests.exceptions.RequestException as e:
